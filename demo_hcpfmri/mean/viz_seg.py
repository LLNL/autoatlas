...
for tidx,tag in enumerate(tags):
    for sub in subjs:
        ...
        log_dir = '/p/gpfs1/mohan3/Data/TBI/HCP/fMRI_flat_images/{}/{}/'.format(tag,test_dirs[tidx])
        ...
        log_dir = '/p/gpfs1/mohan3/Data/TBI/HCP/fMRI_flat_images/proc_768x384/{}/'.format(test_dirs[tidx])
        fMRI = np.asarray(Image.open(os.path.join(log_dir,'{}_rfMRI_REST1_LR_avg.tif'.format(sub))))
        mask = np.asarray(Image.open(os.path.join(log_dir,'{}_rfMRI_REST1_LR_avg_mask.png'.format(sub))))
