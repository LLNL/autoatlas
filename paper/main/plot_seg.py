...
for ac in achans:
    out_folder = os.path.join(log_dir.format(ac),'paper','seg')
    ...
    for i in range(test_start,test_start+num_test):
        ...
        sh = rec.shape
        save_fig(gt[sh[0]//2],'gt{}_aenc{}_z.png'.format(i,ac),cmap='gray')
        save_fig(gt[:,sh[1]//2],'gt{}_aenc{}_y.png'.format(i,ac),cmap='gray')
        save_fig(gt[:,:,sh[2]//2],'gt{}_aenc{}_x.png'.format(i,ac),cmap='gray')
#        save_fig(rec[sh[0]//2],'rec{}_z.png'.format(i,ac),cmap='gray')
#        save_fig(rec[:,sh[1]//2],'rec{}_y.png'.format(i,ac),cmap='gray')
#        save_fig(rec[:,:,sh[2]//2],'rec{}_x.png'.format(i,ac),cmap='gray')
        save_fig(seg[sh[0]//2],'seg{}_aenc{}_z.png'.format(i,ac))
        save_fig(seg[:,sh[1]//2],'seg{}_aenc{}_y.png'.format(i,ac))
        save_fig(seg[:,:,sh[2]//2],'seg{}_aenc{}_x.png'.format(i,ac))
