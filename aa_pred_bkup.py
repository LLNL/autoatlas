...
import os
import csv
from sklearn.linear_model import LogisticRegression,LinearRegression,RANSACRegressor,LassoCV,LogisticRegressionCV,RidgeCV
...
def train_linear(tag,train_input,train_output,train_mask,test_input,test_output,test_mask):
    ...
    if tag == 'Gender' or tag == 'Age' or 'NEORAW_' in tag:
        if ARGS.opt == 'lin' or ARGS.opt == 'ransac' or ARGS.opt=='lasso':
            ...
            classifier = LogisticRegressionCV(refit=True,cv=5,multi_class='multinomial',solver='lbfgs',max_iter=10000).fit(train_input[train_mask],train_output)
        ...
        print(classifier.score(train_input[train_mask],train_output),classifier.score(test_input[test_mask],test_output))
    else:
        if ARGS.opt == 'lin':
            #classifier = LinearRegression(normalize=True).fit(train_input[train_mask],train_output)
            classifier = RidgeCV(cv=5,alphas=(1e-4,1e-3,1e-2,1e-1,1,1e2,1e3)).fit(train_input[train_mask],train_output)
        elif ARGS.opt == 'lasso':
            classifier = LassoCV(cv=5,max_iter=10000).fit(train_input[train_mask],train_output)
...
#    print(rid)
    ...
    train_pred[rid],test_pred[rid] = train_linear(tag,train_in,train_out,train_mask,test_in,test_out,test_mask)
