...
score_mape = {opt:[] for opt in opts_labels.keys()}
...
with open(os.path.join(test_dir,'fs_strun_summ.csv'),'r') as csv_file:
    ...
    for row in reader:
        for opt in opts_labels.keys():
            ...
            if row_label == 'score mape':
                score_mape[opt].append(float(row[opt]))
...
for opt in opts_labels.keys():
    ...
    for r2 in score_r2[opt]:
        if r2 == val:
            text += ' & \\textbf{{{:.2f}}} '.format(r2)
        else:
            text += ' & {:.2f} '.format(r2)
    
    val = min(score_mape[opt])
    for mape in score_mape[opt]:
        if mape == val:
            text += ' & \\textbf{{{:.2f}}} '.format(mape)
        else:
            text += ' & {:.2f} '.format(mape)
    text += '\\\\\\hline'
    print(text)
